...
long **dimatrix(long nrl, long nrh, long ncl, long nch)
/* allocate a int matrix with subscript range m[nrl..nrh][ncl..nch] */
{
	long i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
	long **m;
...
void free_divector(long *v, long nl, long nh)
/* free a long vector allocated with divector() */
{
	free((FREE_ARG) (v+nl-NR_END));
}
...
void free_dimatrix(long **m, long nrl, long nrh, long ncl, long nch)
/* free an int matrix allocated by dimatrix() */
{
	free((FREE_ARG) (m[nrl]+ncl-NR_END));
	free((FREE_ARG) (m+nrl-NR_END));
}
