...
void depositsnow(celltype **cells,int i,int j,double sc,double cc)
{
  ...
  for (k=1;k<nne;k++) {
    ...
    /*if conditions for for avalanching*/
    if ((maxslope > sc)||(curv < cc)) {

      cells[i][j].avasite = 1;

      /*if receiver is not already a donor - avoiding endless loops*/ 
...
void avalanche(celltype **cells,meshtype mesh,mproptype mprop,sortarraytype *sortarray)
{
...
void mass_balance(celltype **cells,meshtype *mesh,iproptype iprop,mproptype mprop,hwproptype hwprop,double time,double dt)
{
  ...
  int dolakeablation = (*mesh).dolakeablation;
  double g = (*mesh).gravity;
  double L = iprop.latentheat; 
  double Ldebris = mprop.Ldebris;
  double maxacc = mprop.maxacc;
  double lakelevel = mprop.lakelevel;
  double floatfrac = mprop.floatfrac;
  double rho_i = 980;
  double rho_w = 1000;

#pragma omp parallel shared(cells,mesh,mprop,hwprop,dice,meanice_b) private(i,j,melt,smelt,bmelt,imelt,accumulation,hs) firstprivate(nx,ny,time,dt,g,L,doglacialsedi,dodebrisablation,dolakeablation,Ldebris,maxacc,lakelevel,floatfrac)
  {
...
#pragma omp for schedule(static)
	...
	if (dolakeablation && cells[i][j].margin == 1 && cells[i][j].bed <= lakelevel && cells[i][j].ice < ((rho_i/rho_w) * (1 + floatfrac) * (lakelevel - cells[i][j].bed))) {
    cells[i][j].lakeablation = cells[i][j].ice;	/*this tracks the ice thickness lost in each cell per dt*/
	cells[i][j].areacalved = 1;	/*this tracks the cells where ice is lost to calving per dt*/
	cells[i][j].ice = 0;
	}
	...
	hs = cells[i][j].ssedi; /*this is set in particles.c*/ 
	if (hs > 2.0) hs = 2.0;
	if (hs < 0.0) hs = 0.0;
...
void get_change(celltype **cells,hptype **hp,vptype **vp,meshtype *mesh,double dt)
{
  ...
  double dH,hdiff;
  double meanice = 0.0;
  double totallake = 0.0;
  double totalcalved = 0.0;
...
#pragma omp single
  {
    ...
    (*mesh).meanice = meanice;
      ...
      /*compute total ice thickness and area lost to lake ablation*/
#pragma omp for schedule(static) reduction(+:totallake) reduction(+:totalcalved)
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
      totallake += cells[i][j].lakeablation * (dx * dy);
	  totalcalved += cells[i][j].areacalved * (dx * dy);
    }
  }
#pragma omp single
  {

    /*totallake /= (double)((*mesh).nc);*/
    (*mesh).totallake = totallake;
    (*mesh).totalcalved = totalcalved; 
