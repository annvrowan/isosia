...
if showtime,
  ...
  xlabel('time'); ylabel('Nit'); title('# of velocity iterations');
  hold off;
  ...
  subplot(3,3,6);
  hold on; grid on; box on;
  yyaxis left; plot(tdata(:,1),tdata(:,36)/1e9); ylabel('Volume lost (km^3)'); 
  yyaxis right; plot(tdata(:,1),tdata(:,37)/1e6); ylabel('Area lost (km^2)'); 
  xlabel('time'); title('Ice lost to lake ablation');
  hold off;
