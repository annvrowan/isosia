...
mproptype read_mprop(char *path,char file[200],FILE *fp)
{
  ...
  fread(&mprop.lakelevel,sizeof(double),1,fp);
  fread(&mprop.floatfrac,sizeof(double),1,fp);
  fclose(fp);
...
void write_output(celltype **cells,hptype **hp,vptype **vp,meshtype mesh,char *path,char file[200],FILE *fp,long step)
{
  ...
  if ((fp = fopen(file,"wb")) == NULL) {
    ...
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].lakeablation,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].areacalved,sizeof(double),1,fp);
