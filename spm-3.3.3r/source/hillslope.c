...
void hillslope(celltype **cells,hptype **hp,vptype **vp,meshtype *mesh,hproptype hprop,double dt)
{

  int i,j;
  double sdiff,dHs,mean_dHs=0.0,fac,ero,maxero;
  ...
  int nx = (*mesh).nx;
  int ny = (*mesh).ny;
  int doerosion = (*mesh).dohillslopeerosion;
  int dosediment = (*mesh).dosediment;

#pragma omp parallel shared(cells,hp,vp,meanerate) private(i,j,sdiff,dHs,fac,maxero,ero) firstprivate(nx,ny,dx,dy,dt,Ks,sc,doerosion,dosediment,Ke,Ls)
  {
...
#pragma omp for schedule(static)
      for (i=0;i<ny;i++) {
	for (j=1;j<nx;j++) {
	  fac = pow(hp[i][j].dbdx/sc,2.0); if (fac > 0.99999) fac = 0.99999;
	  sdiff = Ke/(1.0-fac);
	  ero = sdiff*hp[i][j].dbdx*dt/dx;
#pragma omp critical
	  {
	  if (ero < 0.0) {
...
#pragma omp for schedule(static)
      for (j=0;j<nx;j++) {
	for (i=1;i<ny;i++) {
	  fac = pow(vp[i][j].dbdy/sc,2.0); if (fac > 0.99999) fac = 0.99999;
	  sdiff = Ke/(1.0-fac);
	  ero = sdiff*vp[i][j].dbdy*dt/dy;
#pragma omp critical
	  {
	  if (ero < 0.0) {
...
#pragma omp for schedule(static)
      for (i=0;i<ny;i++) {
	for (j=1;j<nx;j++) {
	  fac = pow(hp[i][j].dtdx/sc,2.0); if (fac > 0.999) fac = 0.999;
	  sdiff = Ks/(1.0-fac);
	  /*dHs = -.5*sdiff*hp[i][j].dbdx*(cells[i][j-1].sedi+cells[i][j].sedi)*dt/dx;*/
	  dHs = -.5*sdiff*hp[i][j].dtdx*dt/dx;
	  if (dHs < 0.0) {
...
#pragma omp for schedule(static)
      for (j=0;j<nx;j++) {
	for (i=1;i<ny;i++) {
	  fac = pow(vp[i][j].dtdy/sc,2.0); if (fac > 0.999) fac = 0.999;
	  sdiff = Ks/(1.0-fac);
	  /*dHs = -.5*sdiff*vp[i][j].dbdy*(cells[i-1][j].sedi+cells[i][j].sedi)*dt/dy;*/
	  dHs = -.5*sdiff*vp[i][j].dtdy*dt/dy;
	  if (dHs < 0.0) {
	    if (cells[i][j].sedi <= 0.0) dHs = 0.0; 
	    else if (dHs < -cells[i][j].sedi) dHs = -cells[i][j].sedi;
	  }
...
#pragma omp for schedule(static) reduction(+:mean_dHs) reduction(+:meanerate)
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
