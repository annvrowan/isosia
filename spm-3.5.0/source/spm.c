...
typedef struct {
  ...
  double meltrate,Mb,Ms,accrate,smelt,mrate,Kgw,tprop,sfac,lee,dhw,te_new,afac,angular,lakeablation,areacalved;
...
typedef struct {
  ...
  int dofluvial,doice,dosliding,doperiglacial,doglacialhydrology,doglacialsedi,doparticles,dodeposit,dodebrisablation,dolakeablation;
  int doglacialerosion,doavalanche,doorographic,dohillslope,dohillslopeerosion,dosediment,coldbased;
  ...
  double meanice,meanele,maxspeed,maxb,maxs,meanice_b,totallake,totalcalved;
...
typedef struct {
  ...
  double qb,Ldebris,dL,precip,cp,ch,lakelevel,floatfrac;
...
#pragma omp for schedule(static)
  ...
  cells[i][j].lakeablation = 0.0;
  cells[i][j].areacalved = 0.0;
...
int main(int argc,char** argv) {
  ...
  while (time < mesh.maxtime)
	...
	if (mesh.doavalanche > 0) avalanche(cells,mesh,mprop,sortarray);
	...
	fprintf(fg,"%4.4e ",Hi);
	fprintf(fg,"%4.4e ",Hq);
  fprintf(fg,"%4.4e ",mesh.totallake);
  fprintf(fg,"%4.4e ",mesh.totalcalved);
