...
SPM.mesh.dofluvial = 0;     %fluvial flag
...
SPM.mesh.doavalanche = 0;   %avalanche flag
...
SPM.mesh.doparticles = 0;   %use lagrangian particles to advect sediment
SPM.mesh.dodeposit = 0;     %allows particles to form sediment when killed
SPM.mesh.dodebrisablation = 0; %reduce surface ablation rates due to debris cover
SPM.mesh.dolakeablation = 0; %adjust ablation for ice-contact lake processes (calving)
...
SPM.iprop.Cs = 1e-3;            %Weertman sliding parameter
...
SPM.iprop.mf = 2.0;             %bedrock fracture density for quarrying
...
SPM.iprop.ksg = 1e-7;           %apparent conductivity of the array of debris to ice 

%***** mprop data *******
SPM.mprop.mtype = 2;            %Type of mass balance function to use %1 = elevation dependent, 2 = temperature dependent, 3 = gridded
SPM.mprop.avaslope = 0.25;      %max stable slope for avalanches
SPM.mprop.avacurv = -0.1;       %min stable curvature for avalanches (wind)
SPM.mprop.maxacc = 100.0;       %maximum accumulation rate
SPM.mprop.dhice = 1.0;          %ice contribution to elevation used to compute mass balance
SPM.mprop.lrate = 0.006;        %lapse rate for air temperature with elevation
SPM.mprop.mPDD = 1e-3;          %Positive degree day factor (see massbalance.m)
SPM.mprop.dTa = 8.0;            %Amplitude of seasonal air temperature (see massbalance.m)
SPM.mprop.qb = 0.045;           %crustal heat flux
SPM.mprop.Ldebris = 0.72;       %ablation reduction beneath debris cover factor (h0; see Rowan et al. 2021, JGR-ES)
SPM.mprop.Temp = [0;10];        %t;T data - temperature at sea level
SPM.mprop.Mrate_h = [5000,6000;-1,1];   %[h;Mrate] data - use with mprop=1
SPM.mprop.Mrate_T = [-10,10;1,1;1,1];   %[T;acc;melt] data - use with mprop=2
SPM.mprop.dL = 10e3;        %smoothing length scale for orographic precip
SPM.mprop.precip = 1.0;     %precipitation rate
SPM.mprop.cp = 2.0e-3;      %precipitation constant
SPM.mprop.ch = 1.0e-6;      %orographic constant
SPM.mprop.lakelevel = 1000;      %elevation of ice-contact lake surface
SPM.mprop.floatfrac = 0.15;      %fraction of ice thickness above floation level

%****** hwprop data ******
SPM.hwprop.a2w = 1;         %fraction of ablation to glacial water 
SPM.hwprop.tscale = 1;      %time scaling < 1 slows system and increase timesteps
SPM.hwprop.kh = 0.1506;     %run code htest.m to find these
SPM.hwprop.h0 = 0.1;  
SPM.hwprop.ds = 0.25;
SPM.hwprop.ss0 = 0.3;
SPM.hwprop.kmin = 1e-7;    %min transmissivity for cavities 
...
% The two latter parameters are used to form sediment along margins without production of sediment in the landscape
...
if SPM.mesh.doperiglacial,
  ...
  SPM.pgprop.minsedi = 1.0;    %expected minimum sediment cover on slopes smaller than minslope
  end;
