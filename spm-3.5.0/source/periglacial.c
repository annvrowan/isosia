...
double get_periglacial_erosion_rate_int(double T0,pgproptype pgprop)
{

  /*uses linear interpolation*/

  int j;
  double erate=0.0,jfac;
  ...
  if ((j == 0)||(j == nT0-1)) erate = pgprop.CiT[j];
  else { 
    jfac = (T0-pgprop.T0v[j-1])/(pgprop.T0v[j]-pgprop.T0v[j-1]);
    erate = (1.0-jfac)*pgprop.CiT[j-1] + jfac*pgprop.CiT[j];
  }
...
void periglacial_nosedi(celltype **cells,hptype **hp,vptype **vp,meshtype *mesh,mproptype mprop,pgproptype pgprop,double dt)
{
...
#pragma omp parallel shared(cells,hp,vp,pgprop,meanerate,mean_dHs) private(i,j,hdiff,sdiff,dHs,Ts,kappa) firstprivate(nx,ny,rho_b,rho_s,dt,lrate,T0,dT,maxsedi,maxice,dosediment)
  {
...
#pragma omp for schedule(static)
    for (i=0;i<ny;i++) {
	  ...
	  Ts = T0 - lrate*cells[i][j].topsedi;
	  /*Ts = cells[i][j].phi + dT;*/
	  if ((cells[i][j].sedi < maxsedi)&&(cells[i][j].ice < maxice)&&(cells[i][j].fixflag <= 0)) cells[i][j].periglacial_erate = get_periglacial_erosion_rate_int(Ts,pgprop);
	  else cells[i][j].periglacial_erate = 0.0;
	  cells[i][j].periglacial_erosion += cells[i][j].periglacial_erate*dt;
	  cells[i][j].bed -= cells[i][j].periglacial_erate*dt;
	  if (dosediment > 0) cells[i][j].sedi += rho_b/rho_s*cells[i][j].periglacial_erate*dt;
	}
      } 
